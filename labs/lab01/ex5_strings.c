...
int main() {
  ...
  // DONE: store the null terminator
  hello_str[5] = 0; // '\0';

  // Prints hello_str
  printf("prints hello: %s\n", hello_str);

  // DONE: print the length of hello_str
  ...
  // DONE: compute the address of the letter r using world_str
  ...
  // DONE: store the space character in "hello world" at the correct index
  // Note: a space is not the same as null terminator
  //       a null terminator is represented by '\0'
  hello_world_str[strlen(hello_str)] = ' ';

  // DONE: use strcpy, pointer arithmetic, and world_str to store
