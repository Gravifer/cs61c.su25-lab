...
// DONE: create a pointer to input
void add_one_pointer(int* input) {
  // DONE: add one to the integer that input points to
  *input += 1;
}

// DONE: create a pointer to a pointer to input
void add_one_double_ptr(int** input) {
  // DONE: add one to the integer that input doubly points to
  **input += 1;
}
...
int main() {
  ...
  // DONE: use add_one_pointer to increment x
  ...
  // DONE: Let's save the pointer to x in y
  int* y = &x;

  // DONE: use add_one_double_ptr to increment x again, using the pointer we just made
